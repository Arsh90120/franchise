...
export default function RosterPage() {
  ...
  const { selectedTeam, isSetupComplete } = useGameState();
  ...
  useEffect(() => {
    ...
    fetch(`/api/roster/${selectedTeam.id}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  }, [selectedTeam, isSetupComplete, router]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <h1 className="font-heading text-4xl font-800 uppercase mt-1">
          {selectedTeam?.city} <span className="text-orange">{selectedTeam?.name}</span> Roster
        </h1>
        <p className="text-muted text-sm font-body mt-1">2024–25 Season · {players.length} Players</p>
      </div>

      <Card>
        {loading ? (
          <p className="text-muted text-sm font-body py-8 text-center">Loading roster...</p>
        ) : (
              ...
              <thead>
                <tr className="border-b border-border">
                  {['#', 'Player', 'Pos', 'PPG', 'RPG', 'APG', 'FG%', 'MIN'].map((h) => (
                    <th key={h} className="text-left text-muted font-body font-400 py-2 pr-4 last:pr-0">
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  return (
                    <tr key={p.id} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                      ...
                      <td className="py-3 pr-4">
                        <span className="font-heading font-700 text-text">
                      ...
                      <td className="py-3 pr-4 font-heading font-700">{avg?.pts?.toFixed(1) ?? '—'}</td>
                      <td className="py-3 pr-4 font-heading font-700">{avg?.reb?.toFixed(1) ?? '—'}</td>
                      <td className="py-3 pr-4 font-heading font-700">{avg?.ast?.toFixed(1) ?? '—'}</td>
